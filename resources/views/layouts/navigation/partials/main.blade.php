...
<x-nav-link :href="route('customers.index')"
    ...
    >
        <svg
            ...
            stroke-width="1.5"
            stroke="currentColor"
            class="hi-outline hi-calendar-days inline-block size-5"
        >
            <path
    ...
    <span class="grow py-1">{{ __('Customers') }}</span>
</x-nav-link>

<x-nav-link :href="route('prices.index')"
            :active="request()->routeIs('prices.index')"
            wire:navigate>
    <span
        class="flex flex-none items-center text-gray-400 group-hover:text-blue-500 dark:text-gray-500 dark:group-hover:text-gray-300"
    >
    ...
    <span class="grow py-1">{{ __('Price') }}</span>
</x-nav-link>
...
@can('createCustomer', [auth()->user()->team])
    <x-nav-link :href="route('customers.create')"
        ...
        <span class="grow py-1">{{ __('Create customer') }}</span>
    </x-nav-link>
@endcan
